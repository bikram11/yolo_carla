...
def p3d_to_p2d_bb(p3d_bb):
    min_x = np.amin(p3d_bb[:,0])/960
    min_y = np.amin(p3d_bb[:,1])/540
    max_x = np.amax(p3d_bb[:,0])/960
    max_y = np.amax(p3d_bb[:,1])/540
...
def get_vehicle_class(vehicles, json_path=None):
    ...
    class_list = []
    for v in vehicles:
        v_class = int(vehicles_data.get(v.type_id))
...
def save_output(carla_img, bboxes, vehicle_class):
    ...
    if not os.path.exists(os.path.dirname(txt_folder)):
        os.makedirs(os.path.dirname(txt_folder))    
        
    carla_img.save_to_disk(f'{image_folder}/%06d.png'%carla_img.frame,carla.ColorConverter.Raw)
    bboxes_list = [bbox.tolist() for bbox in bboxes]
    filename = f'{txt_folder}/%06d.txt' % carla_img.frame

    # Ensure the directory exists
    if not os.path.exists(os.path.dirname(filename)):
        os.makedirs(os.path.dirname(filename))

    # Accumulate all lines to write
    lines_to_write = []
    for i in range(len(vehicle_class)):
        combined_values = f"{vehicle_class[i]} " + " ".join(map(str, bboxes_list[i]))
        lines_to_write.append(combined_values)
...
class ClientSideBoundingBoxes(object):
    ...
    def get_bounding_boxes(vehicles, camera,carla_rgb):
        ...
        """
        vehicles = carla_vehicle_annotator.filter_angle_distance(vehicles , camera, 100)
